...
const passwordReducer = (state, action) => {
  if (action.type === 'USER_INPUT') {
    return {
      value: action.val,
      isValud: action.val.trim().length > 6,
    };
  }else if(action.type ==='INPUT.VALIDATE'{
    value:state.value,
...
const Login = ({ onLogin }) => {
  ...
  */
  ...
  const [] = useReducer(passwordReducer, {
    value: '',
    isValid: null,
  });
  ...
  //emailState에서 isValid 프로퍼티를 디스트럭처링함(프로퍼티로 바로 사용x)
  ...
  const emailChangeHandler = (e) => {
    ...
    //dispatch함수의 매개값 객체의 key는 정해진 것이 아닌,reduce 함수에서
    //구분하기 위해 붙여주는 이름
    //properties의 key와 value는 자유롭게 줄 수 있다(정해진게 아니다)
    dispatchEmail({
      type: 'USER_INPUT',
      val: e.target.value,
    });
  ...
  const submitHandler = (e) => {
    e.preventDefault();
    onLogin(emailIsValid, enteredPassword);
  };

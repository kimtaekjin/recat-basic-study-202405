...
const Login = ({ onLogin }) => {
  ...
  useEffect(() => {
    //formIsValid의 유효성 검증을 일부러 1초뒤에 실행하도록setTimeout을 사용.
    //1초 이내에 새로운 입력값이 들어옴 -> 상태 변경 ->재 렌더링이 진행되면서
    //useEffect가 또 호출됨.
    const timer = setTimeout(() => {
      console.log('useEffect called in Login.Js');
      //의존성 배열에 상태변수를 넣어주면 그 상태변수가 바뀔 때마다
      //useEffect가 재실행됨.
      setFormIsValid(
    ...
    //사용자가 1초 이내에 추가 입력 ->상태 변경 ->위에 예약한 timer취소
    return () => {
